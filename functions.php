...
function twentytwelve_get_font_url() {
	...
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'twentytwelve' ) ) {
		...
		$font_url = add_query_arg( $query_args, 'https://fonts.233.wiki/css' );
	}
...
function twentytwelve_scripts_styles() {
	...
	wp_enqueue_style( 'zivers', get_stylesheet_uri() );

	// Loads the Internet Explorer specific stylesheet.
	wp_enqueue_style( 'zivers-ie', get_template_directory_uri() . '/css/ie.css', array( 'twentytwelve-style' ), '20121010' );
	$wp_styles->add_data( 'zivers-ie', 'conditional', 'lt IE 9' );
}
...
function twentytwelve_entry_meta() {
	...
	if ( $tag_list ) {
		...
		$utility_text = __( '作者:%4$s | 分类:%1$s | 标签:%2$s | 日期:%3$s', 'twentytwelve' );
	} elseif ( $categories_list ) {
		#$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="by-author"> by %4$s</span>.', 'twentytwelve' );
		$utility_text = __( '作者:%4$s | 分类:%1$s | 日期:%3$s', 'twentytwelve' );
	} else {
		#$utility_text = __( 'This entry was posted on %3$s<span class="by-author"> by %4$s</span>.', 'twentytwelve' );
		$utility_text = __( '作者:%4$s | 日期:%3$s', 'twentytwelve' );
	}
...
add_filter('widget_tag_cloud_args', 'z_tag_cloud_filter', 10);

function z_disable_emojis() {
    	...
    	add_filter( 'tiny_mce_plugins', 'z_disable_emojis_tinymce' );
}
add_action( 'init', 'z_disable_emojis' );
